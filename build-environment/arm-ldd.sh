...
# Because of mystery problems, just exit claiming that the file is not a dynamic executable
# This makes linuxdeploy not copy libraries and not segfault for whatever arm emulation reason
# For now just copy manually the libraries
...
while test $# -gt 0; do
  ...
  --vers | --versi | --versio | --version)
    echo 'ldd (Ubuntu GLIBC 2.39-0ubuntu8.3) 2.39'
  ...
  if test ! -e "$file"; then
    ...
    for rtld in ${RTLDLIST}; do
      if test -x $rtld; then
        ...
        #  esac
        #fi
        # Get rid of verification as for some reason ld-linux-aaarch64.so.1 segfaults it with node
        ret=0
        RTLD=${rtld}
      fi
    done
    ...
    1)
      ...
      ;;
    0|2)
      # Idk what change this really makes, but I guess it works as we actually don't want to copy
      # the linker anyways
